...
// GUI 구현을 위해 사용하는 클래스들 (swing, awt)
...
public class Calculator extends JFrame implements ActionListener, KeyListener {
	...
	final int MAX_INPUT_LENGTH = 20;  // 최대 입력 가능한 길이를 제한
	...
	public Calculator(String title) {
		    ...
		    * - 예시 : FlowLayout(FlowLayout.Left, 10, 20); // 왼쪽정렬, 좌우간격 10, 상하간격 20
		    * 2)BorderLayout : 상하좌우 + 중앙 5개의 영역으로 나눠 배치
		    * - 예시 : BorderLayout(30,20); // 좌우간격 30, 상하간격 20
		    * 3)GridLayout : 2차원 그리드로서 n x n 으로 설정, 왼->오, 위->아래 순 배치
		    * - 예시 : GridLayout(2,3,4,5,50); // 4 x 3 그리드, 좌우간격 5, 상하간격 50
	...
	private void processSingChange() {
		...
		if (displayMode == INPUT_MODE){
			String input = getDisplayString(); // 기존 화면에 있는 문자열 저장
